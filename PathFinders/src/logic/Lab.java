...
public class Lab extends Graph{
    ...
    public static Lab[] generateRandomLab(int rows, int columns, int numCopy){
        ...
        for (int i = 0; i < numCopy; i++) {
            ...
            current[i].getNodeInfo().setVisited(true);
            ArrayList<Edge> currentEdges = new ArrayList<>();
            for (int j = 0; j < current[i].getNodeInfo().getNumEdge(); j++) {
        ...
        while (!neighbours.get(0).isEmpty()){
            ...
            for (int i = 0; i < numCopy; i++) {
                ...
                if(!destination.getNodeInfo().isVisited()){
                    ...
                    removeWall(currentEdge);

                    destination.getNodeInfo().setVisited(true);

                    for (int j = 0; j < destination.getNodeInfo().getNumEdge(); j++) {
    ...
    private static void removeWall(Edge currentEdge) {
        Cell source = (Cell) currentEdge.getSource();
        CellInfo sourceInfo = source.getNodeInfo();
        Cell destination = (Cell) currentEdge.getDestination();
        CellInfo destinationInfo = destination.getNodeInfo();

        if(source.getNodeInfo().getY() < destination.getNodeInfo().getY()){
            sourceInfo.disconnect(CellInfo.Connection.RIGHT);
            destinationInfo.disconnect(CellInfo.Connection.LEFT);
        } else if (source.getNodeInfo().getY() > destination.getNodeInfo().getY()) {
            sourceInfo.disconnect(CellInfo.Connection.LEFT);
            destinationInfo.disconnect(CellInfo.Connection.RIGHT);
        }else {
            if (source.getNodeInfo().getX() > destination.getNodeInfo().getX()) {
                sourceInfo.disconnect(CellInfo.Connection.TOP);
                destinationInfo.disconnect(CellInfo.Connection.BOTTOM);
            } else if (source.getNodeInfo().getX() < destination.getNodeInfo().getX()) {
                sourceInfo.disconnect(CellInfo.Connection.BOTTOM);
                destinationInfo.disconnect(CellInfo.Connection.TOP);
            }
        }
